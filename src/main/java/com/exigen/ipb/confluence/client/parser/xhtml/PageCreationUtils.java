...
public class PageCreationUtils {
	...
	public static void createPages(String targetDir, Map<String, PageInfo> pages, Boolean isPublicDoc){
		for (Entry<String, PageInfo> pageEntry : pages.entrySet()) {
			if (isApplicable(isPublicDoc, pageEntry.getValue().isPublicInfoHolder())) {
	...
	private static void createPage(String targetDir, Entry<String, PageInfo> pageEntry, Boolean isPublicDoc){
		Document doc = Jsoup.parse("");
		Element bodyElement = doc.getElementsByTag("body").get(0);
		...
		try {
			...
			throw new EisConfluenceException("Failed creating page file for " + page.getName(), e);
		}
	}
