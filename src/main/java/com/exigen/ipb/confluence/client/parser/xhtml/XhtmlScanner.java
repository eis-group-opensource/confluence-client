...
package com.exigen.ipb.confluence.client.parser.xhtml;
...
public class XhtmlScanner {
     ...
     */
    public static Map<String, PageInfo> scan(File basedir, Log log, List<String> skipDirs) {
     ...
     * @throws EisConfluenceException when any issue occures.
     */
    private static PageInfo extractInfo(File xhtmlFile, Log log) {
        PageInfo pageInfo = new PageInfo();
        boolean containsScopeAnnotation = false;

        String canonicalPath = null;
        try {
            canonicalPath = xhtmlFile.getCanonicalPath();
        } catch (IOException e) {
            throw new EisConfluenceException(e.getMessage(), e);
        }
        pageInfo.setRelativePath(extractRelativePath(canonicalPath));
    ...
    private static boolean parseXhtmlInfo(PageInfo pageInfo, File xhtmlFile, Log log) {
        boolean containsScopeAnnotation = false;

        Document doc = null;
        try {
            doc = Jsoup.parse(xhtmlFile, null);
        } catch (IOException e) {
            throw new EisConfluenceException(e.getMessage(), e);
        }
        ...
        if (compositionTags.size() == 1) {
            ...
            throw new EisConfluenceException("Page " + pageInfo.getRelativePath() + " is corrupt. It contains more than one <ui:composition/> tags.");
        }
    ...
    private static void parseUiIncludes(PageInfo pageInfo, Elements uiIncludeTags, Log log)  {
    ...
    private static void parseEfIncludes(PageInfo pageInfo, Elements efIncludeTags, Log log) {
    ...
    private static void parseUiDecorates(PageInfo pageInfo, Elements uiDecorateTags, Log log)  {
    ...
    private static boolean parseDescriptionAndParams(PageInfo pageInfo, Node node, Boolean innerPageInfo, Log log) {
        boolean containsScopeAnnotation = false;
    ...
    private static void parseParameters(Element uiIncludeTag, PageInfo includeInfo) {
